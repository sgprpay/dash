...
const char * const BITCOIN_CONF_FILENAME = "sgpr.conf";
const char * const BITCOIN_PID_FILENAME = "sgprd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "sgpr" is a composite category enabling all Sgpr-related debug output
            if(ptrCategory->count(string("sgpr"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SgprCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SgprCore
    // Mac: ~/Library/Application Support/SgprCore
    // Unix: ~/.sgprcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SgprCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SgprCore";
#else
    // Unix
    return pathRet / ".sgprcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty sgpr.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override sgpr.conf
